...
st.title("Cambios en el comportamiento de los usuarios de Twitter")
...
st.write("* En este caso los usuarios que se muestran son aquellos que fueron más eficaces y con mayor número de interacciones los últimos 10 días del proceso electoral.")
...
fig.add_trace(go.Bar(
    x=data1['username'],
    y=data1['avg_interaction_recent'],
    name='Últimos 10 días',
    marker_color='#1b9e77'
))
...
fig.update_layout(
    title={
        'text': "Average Interactions per Post by User",
        'y': 0.9,
        'x': 0.5,
        'xanchor': 'center',
        'yanchor': 'top'
...
# Aplicar la plantilla personalizada
plot_template_plotly(
    fig,
...
# Mostrar la gráfica en Streamlit (opcional)
st.plotly_chart(fig)